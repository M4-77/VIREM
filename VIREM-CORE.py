    ...
    try:
        ...
        if not cpu_name:
            with open("/proc/cpuinfo") as f:
                for l in f:
                    if "model name" in l:
                        cpu_name = l.strip().split(":")[1].strip()
                        break   # linux exclusive im pretty sure so if someone can port this to windows that would be great
        s["CPU"] = cpu_name
    except:
        s["CPU"] = "Unknown"
...
# tool functions, the LLM can decide when to call these in the personality-core.txt file, you can add more if you know what youre doing just make sure to let the LLM know how to call it in the personality-core
def create_project(name, directory=DOCUMENTS_DIR):
    p = os.path.join(directory, name)
    try:
        os.makedirs(p, exist_ok=True)
        return f"Project '{name}' created at {p}"
    except Exception as e:
        return f"Error creating project: {e}"


def delete_file(path): # be REALLY careful with this lmao
        ...
        for f in os.listdir(DOWNLOADS_DIR):
            ...
            if os.path.isfile(fp):
                try:
                    ...
                    mtime = None
                if mtime is None or mtime < week_ago:
                    os.remove(fp)
                    removed.append(f)
        return f"Removed files: {removed}" if removed else "No files to remove."
    except Exception as e:
        return f"Error clearing downloads: {e}"
...
def handle_tool_command(cmd):
    if cmd.startswith("open:"):
        return open_application(cmd.split("open:", 1)[1].strip())
    elif cmd.startswith("clear_downloads"):
        return clear_downloads()
    elif cmd.startswith("note:"):
        return write_note(cmd.split("note:", 1)[1].strip())
    elif cmd.startswith("create_project:"):
        return create_project(cmd.split("create_project:", 1)[1].strip())
    elif cmd.startswith("delete:"):
        return delete_file(os.path.join(DOCUMENTS_DIR, cmd.split("delete:", 1)[1].strip()))
    elif cmd.startswith("calc:"):
        return calculator(cmd.split("calc:", 1)[1].strip())
    return f"Unknown tool command: {cmd}"


with open("personality-core.txt", "r", encoding="utf-8") as f:
    personality_prompt = f.read()

# config, i should prob put this either at the top or in a seperate fil
...
Rage = (Hostility / 2) * Corruption / 1.5  # random calculation i made up to increase its rage, not implemented as of 9/9/25
...
while True:
    ...
    if len(message_history) > MAX_HISTORY:
        message_history = message_history[-MAX_HISTORY:]  # removes any history above the max history
    ...
    response = generate("gpt-oss:20b", full_prompt)  # <------------------------------------------------------------    the AI model you pull should be put here, i'll add it as a config once i set that up
    assistant_text = response['response']

    tools = re.findall(r"<tool>(.*?)</tool>", assistant_text, re.DOTALL)
    for t in tools:
        print(f"[VIRE-Tool]: {handle_tool_command(t)}")

    clean_text = re.sub(r"<tool>.*?</tool>", "", assistant_text).strip()
    print(f"VIREM: {clean_text}")

    if clean_text:
        subprocess.run(["espeak", clean_text])  # uses espeak to output, sudo pacman -S espeak (assuming pacman) in terminal if you haven't already as it's pretty cool
